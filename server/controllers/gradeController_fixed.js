...
const saveGradeSheet = async (req, res) => {
  try {
    const { subjectId, term, academicYear } = req.body;
    if (req.body.assessmentTypeId && req.body.scores) {
      ...
      for (const item of scores) {
        ...
        let gradeDoc = await Grade.findOne({ student: item.studentId, subject: subjectId, term, academicYear });
        if (!gradeDoc) gradeDoc = new Grade({ student: item.studentId, subject: subjectId, term, academicYear, assessments:[], finalScore: 0 });
      ...
      return res.status(200).json({ success: true, message: "Grades saved successfully" });
        ...
        let gradeDoc = await Grade.findOne({ student: studentId, subject: subjectId, term, academicYear });
        if (!gradeDoc) gradeDoc = new Grade({ student: studentId, subject: subjectId, term, academicYear, assessments:[], finalScore: 0 });
...
const getGradeDetails = async (req, res) => {
  ...
  try {
    const grade = await Grade.findOne({ student: studentId, subject: subjectId, term, academicYear })
...
const updateGrade = async (req, res) => {
  try {
    ...
    const { assessments, term, academicYear } = req.body;
