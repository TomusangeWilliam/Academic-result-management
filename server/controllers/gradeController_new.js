  ...
  try {
    ...
    for (const data of extractedData) {
      ...
      for (const [subHeader, pdfScore] of Object.entries(data.scores)) {
        ...
        // FIX: find existing grade by term STRING match (not storing a $regex object)
        let gradeDoc = await Grade.findOne({
          student: student._id,
          subject: subId,
          academicYear: currentAcademicYear,
          term: { $regex: new RegExp(currentTerm.split(" ")[0], "i") },
        });

        if (!gradeDoc) {
          // FIX: save actual term string, NOT a $regex operator
          gradeDoc = new Grade({
            student: student._id,
            subject: subId,
            term: currentTerm, // ← actual string
            academicYear: currentAcademicYear,
            assessments: [],
            finalScore: 0,
          });
...
exports.transferMarks = async (req, res) => {
  try {
    ...
    // Get current term and academic year from global config
    const config = await GlobalConfig.findOne();
    const { currentTerm, currentAcademicYear } = config || {};
