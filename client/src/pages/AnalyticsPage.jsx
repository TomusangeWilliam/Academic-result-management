...
const gradeBadge = (pct) => {
  const g = GRADING_SCALE.find(s => pct >= s.min) || GRADING_SCALE[GRADING_SCALE.length - 1];
  return g;
};
    ...
    <div className="bg-white p-6 rounded-lg shadow-md space-y-6 min-h-screen">
      ...
      <div className="p-5 bg-slate-50 rounded-2xl border-2 border-slate-100 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          ...
          <div>
            <label className="text-[10px] font-black text-slate-400 uppercase ml-1 mb-1 block">
              Term <span className="font-normal text-gray-400">(optional)</span>
            </label>
            <select
              value={term}
        ...
        <div className="space-y-8">
          ...
          <section className="bg-gray-50/50 p-4 rounded-2xl border border-gray-100">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-9 gap-3">
                {GRADING_SCALE.map(g => (
                    <div key={g.label} className={`${g.cls} border ${g.border} p-3 rounded-2xl text-center shadow-sm hover:shadow-md transition-shadow`}>
                        <span className="block text-xl font-black mb-0.5">{g.label}</span>
                        <span className="text-[10px] opacity-70 font-black tracking-tight">{g.min}{g.max === 100 ? '+' : ` – ${g.max}`}%</span>
                    </div>
                ))}
            </div>
          </section>
      ...
      <div ref={printRef} style={{ display: "none" }}>
        {r?.analysis && (
          <>
            <h2>
              Subject Analysis — {at?.name} ({at?.month} / {at?.term})
            </h2>
